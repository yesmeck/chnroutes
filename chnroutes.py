...
def generate_iptables(metric):
  ...
  #Ignore all chinese ips
  """)

  script_footer=textwrap.dedent("""\
  #Anything else should be redirect
  iptables -t nat -A SHADOWSOCKS -p tcp -j REDIRECT --to-ports 8090

  # Apply the rules
  iptables -t nat -A PREROUTING -p tcp -j SHADOWSOCKS
  """)

  rulefile=open('rule.sh', 'w')

  rulefile.write(script_header)

  for ip,mask,_ in results:
    rulefile.write('iptables -t nat -A SHADOWSOCKS -p tcp -d %s/%s -j RETURN\n'%(ip,mask))

  rulefile.write('\n')
  rulefile.write(script_footer)
  rulefile.write('\n')

  print "Replace server_ip to you shadowsocks server id, and run rule.sh."
...
if __name__=='__main__':
    ...
    parser.add_argument('-p','--platform',
                        ...
                        "win, android, iptables. openvpn by default.")
    ...
    elif args.platform.lower() == 'iptables':
        generate_iptables(args.metric)
    else:
